...
code_template = """\
    ...
    std::uint8_t digest{i}[SHA256::DIGEST_SIZE] = {digest_val};
    ctx.reset();
    ctx.do_final(digest, msg{i}, sizeof(msg{i}));
    if (std::memcmp(digest{i}, digest, SHA256::DIGEST_SIZE) != 0)
    {{
        throw std::runtime_error("err in sha256");
    }}"""
...
for i in range(TEST_VECTOR_NUM):
    ...
    digest = hashlib.new("sha256", msg).digest()
    code.append(
...
with open("test_sha256.cpp", "w") as fp:
