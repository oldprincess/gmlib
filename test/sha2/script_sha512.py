...
code_template = """\
    ...
    std::uint8_t digest{i}[SHA512::DIGEST_SIZE] = {digest_val};
    ctx.reset();
    ctx.do_final(digest, msg{i}, sizeof(msg{i}));
    if (std::memcmp(digest{i}, digest, SHA512::DIGEST_SIZE) != 0)
    {{
        throw std::runtime_error("err in sha512");
    }}"""
...
for i in range(TEST_VECTOR_NUM):
    ...
    digest = hashlib.new("sha512", msg).digest()
    code.append(
...
with open("test_sha512.cpp", "w") as fp:
