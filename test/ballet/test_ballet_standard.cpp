...
void test_ballet_standard()
{
    BalletCTX ctx;
    uint8_t   out[32];
    // ====================== 128128
    ballet128128_enc_key_init(&ctx, key128128);
    ballet128128_enc_block(&ctx, out, pt128128);
    if (memcmp(out, ct128128, BALLET128128_BLOCK_SIZE) != 0)
    {
        throw runtime_error("err in ballet standard");
    }
    memcpy(out, pt128128, BALLET128128_BLOCK_SIZE);
    for (int i = 0; i < 100; i++)
    {
        ballet128128_enc_block(&ctx, out, out);
    }
    if (memcmp(out, ct128128_100, BALLET128128_BLOCK_SIZE) != 0)
    {
        throw runtime_error("err in ballet standard");
    }
    memcpy(out, pt128128, BALLET128128_BLOCK_SIZE);
    ...
    if (memcmp(out, ct128128_10000, BALLET128128_BLOCK_SIZE) != 0)
    {
        throw runtime_error("err in ballet standard");
    }
    ballet128128_dec_key_init(&ctx, key128128);
    ballet128128_dec_block(&ctx, out, ct128128);
    if (memcmp(out, pt128128, BALLET128128_BLOCK_SIZE) != 0)
    {
        throw runtime_error("err in ballet standard");
    }
    // ===================== 128256
    ballet128256_enc_key_init(&ctx, key128256);
    ballet128256_enc_block(&ctx, out, pt128256);
    if (memcmp(out, ct128256, BALLET128256_BLOCK_SIZE) != 0)
    {
        throw runtime_error("err in ballet standard");
    }
    memcpy(out, pt128256, BALLET128256_BLOCK_SIZE);
    ballet128256_dec_key_init(&ctx, key128256);
    ballet128256_dec_block(&ctx, out, ct128256);
    if (memcmp(out, pt128256, BALLET128256_BLOCK_SIZE) != 0)
    {
        throw runtime_error("err in ballet standard");
    }
    // ===================== 256256
    ballet256256_enc_key_init(&ctx, key256256);
    ballet256256_enc_block(&ctx, out, pt256256);
    if (memcmp(out, ct256256, BALLET256256_BLOCK_SIZE) != 0)
    {
        throw runtime_error("err in ballet standard");
    }
    memcpy(out, pt256256, BALLET256256_BLOCK_SIZE);
    ballet256256_dec_key_init(&ctx, key256256);
    ballet256256_dec_block(&ctx, out, ct256256);
    if (memcmp(out, pt256256, BALLET256256_BLOCK_SIZE) != 0)
    {
        throw runtime_error("err in ballet standard");
    }
}
