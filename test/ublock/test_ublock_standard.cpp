...
void test_ublock_standard()
{
    uBlockCTX ctx;
    uint8_t   out[32];
    // ====================== 128128
    ublock128128_enc_key_init(&ctx, key128128);
    ublock128128_enc_block(&ctx, out, pt128128);
    if (memcmp(out, ct128128, UBLOCK128128_BLOCK_SIZE) != 0)
    {
        throw runtime_error("err in ublock standard");
    }
    memcpy(out, pt128128, UBLOCK128128_BLOCK_SIZE);
    for (int i = 0; i < 1000000; i++)
    {
        ublock128128_enc_block(&ctx, out, out);
    }
    if (memcmp(out, ct128128_1000000, UBLOCK128128_BLOCK_SIZE) != 0)
    {
        throw runtime_error("err in ublock standard");
    }
    ublock128128_dec_key_init(&ctx, key128128);
    ublock128128_dec_block(&ctx, out, ct128128);
    if (memcmp(out, pt128128, UBLOCK128128_BLOCK_SIZE) != 0)
    {
        throw runtime_error("err in ublock standard");
    }
    // ===================== 128256
    ublock128256_enc_key_init(&ctx, key128256);
    ublock128256_enc_block(&ctx, out, pt128256);
    ...
    for (int i = 0; i < 1000000; i++)
    {
        ublock128256_enc_block(&ctx, out, out);
    }
    if (memcmp(out, ct128256_1000000, UBLOCK128256_BLOCK_SIZE) != 0)
    {
        throw runtime_error("err in ublock standard");
    }
    ublock128256_dec_key_init(&ctx, key128256);
    ublock128256_dec_block(&ctx, out, ct128256);
    if (memcmp(out, pt128256, UBLOCK128256_BLOCK_SIZE) != 0)
    {
        throw runtime_error("err in ublock standard");
    }
    // ===================== 256256
    ublock256256_enc_key_init(&ctx, key256256);
    ublock256256_enc_block(&ctx, out, pt256256);
    ...
    for (int i = 0; i < 1000000; i++)
    {
        ublock256256_enc_block(&ctx, out, out);
    }
    if (memcmp(out, ct256256_1000000, UBLOCK256256_BLOCK_SIZE) != 0)
    {
        throw runtime_error("err in ublock standard");
    }
    ublock256256_dec_key_init(&ctx, key256256);
    ublock256256_dec_block(&ctx, out, ct256256);
