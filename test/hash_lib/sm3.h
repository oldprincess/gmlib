...
static void sm3_fast_init(Sm3FastCTX* ctx)
{
...
static void sm3_fast_reset(Sm3FastCTX* ctx)
{
    sm3_fast_init(ctx);
}

static int sm3_fast_update(Sm3FastCTX* ctx, const uint8_t* in, size_t inl)
{
    if (inl > UINT64_MAX / 8)
    {
...
static void sm3_fast_final(Sm3FastCTX* ctx, uint8_t digest[32])
{
...
private:
    ...
    {
        if (sm3_internal::sm3_fast_update(&ctx_, in,
                                          block_num * SM3_BLOCK_SIZE))
        {
    ...
    {
        if (sm3_internal::sm3_fast_update(&ctx_, in, inl))
        {
            throw std::runtime_error("err in sm3 update blocks");
        }
        sm3_internal::sm3_fast_final(&ctx_, digest);
    }
};
