...
{
    std::size_t n;
    asn1_encode_tl(out, &n, ASN1_TAG_CLASS_UNIVERSAL, ASN1_TAG_PC_PRIMITIVE,
                   ASN1_TAG_OBJECT_IDENTIFIER, data_len);
    std::memmove(out + n, data, data_len);
    *outl = n + data_len;
}
...
void asn1_encode_explicit_tl(std::uint8_t* out,
                             ...
                             std::size_t   value_length) noexcept
{
    asn1_encode_tl(out, outl, ASN1_TAG_CLASS_CONTEXT_SPECIFIC,
                   ASN1_TAG_PC_CONSTRUCTED,
                   static_cast<Asn1TagNumber>(tag_number), value_length);
}
                              ...
                              std::size_t         inl) noexcept
{
    ...
    if (tlv.tag_class != ASN1_TAG_CLASS_UNIVERSAL ||
        tlv.tag_pc != ASN1_TAG_PC_PRIMITIVE ||
                                 ...
                                 std::size_t          inl) noexcept
{
    ...
    if (tlv.tag_class != ASN1_TAG_CLASS_UNIVERSAL ||
        tlv.tag_pc != ASN1_TAG_PC_PRIMITIVE ||
        tlv.tag_number != ASN1_TAG_BIT_STRING || tlv.length == 0)
    {
        return -1;
    }
    // contents
    ...
    *data_ptr    = tlv.value + 1;
    *data_length = tlv.length - 1;
    *bits_length = 8 * (tlv.length - 1) - tlv.value[0];
    return 0;
}
                                   ...
                                   std::size_t          inl) noexcept
{
    ...
    if (tlv.tag_class != ASN1_TAG_CLASS_UNIVERSAL ||
        tlv.tag_pc != ASN1_TAG_PC_PRIMITIVE ||
        tlv.tag_number != ASN1_TAG_OCTET_STRING || tlv.length == 0)
    {
        return -1;
    }
    // contents
...
{
    Asn1TLV tlv;
    if (asn1_decode_tlv(&tlv, read_num, in, inl) ||
        tlv.tag_class != ASN1_TAG_CLASS_UNIVERSAL ||
        tlv.tag_pc != ASN1_TAG_PC_PRIMITIVE ||
        tlv.tag_number != ASN1_TAG_OBJECT_IDENTIFIER || tlv.length == 0)
    {
        return -1;
    }
    *data_ptr    = tlv.value;
                               ...
                               std::size_t          inl) noexcept
{
    Asn1TLV tlv;
    if (asn1_decode_tlv(&tlv, read_num, in, inl) ||
        tlv.tag_class != ASN1_TAG_CLASS_CONTEXT_SPECIFIC ||
        tlv.tag_pc != ASN1_TAG_PC_CONSTRUCTED ||
        static_cast<unsigned int>(tlv.tag_number) != tag_number)
    {
        return -1;
    }
    *value_ptr    = tlv.value;
