...
const CipherInfo SM4Info = {
    .init = (CipherInit)sm4_init,
    .encrypt = (CipherEncrypt)sm4_encrypt,
    .decrypt = (CipherDecrypt)sm4_decrypt,
};
...
void sm4_init(uint8_t* key, SM4_CTX* ctx) {
    ...
    for (int i = 0; i < 32; i++) {
        ...
        // L
        ctx->rk[i] = k[0] ^ tmp ^ rotl(tmp, 13) ^ rotl(tmp, 23);
...
static void sm4_do(uint8_t* out, uint8_t* in, SM4_CTX* sm4key, int enc) {
    uint32_t x[4];
    uint32_t tmp, tmp1;
    ...
    for (int i = 0; i < 32; i++) {
        ...
        //查找表（S盒+线性变换L）
        tmp = x[0];
        tmp ^= Table0[(tmp1 >> 24) & 0xFF];
        tmp ^= Table1[(tmp1 >> 16) & 0xFF];
        tmp ^= Table2[(tmp1 >> 8) & 0xFF];
        tmp ^= Table3[(tmp1 >> 0) & 0xFF];
