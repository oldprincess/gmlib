                         ...
                         std::uint64_t     T[8]) noexcept
{
    ...
    T8 = 0;
    // step 1
    m            = (std::uint64_t)(T[0] * N_);
    m_times_p[0] = _mulx_u64(P[0], m, &hi[0]);
    m_times_p[1] = _mulx_u64(P[1], m, &hi[1]);
    m_times_p[2] = _mulx_u64(P[2], m, &hi[2]);
    m_times_p[3] = _mulx_u64(P[3], m, &hi[3]);
    ...
    if (T8 || uint256_cmp(r, P) >= 0)
    {
        CF = _subborrow_u64(0, r[0], P[0], &r[0]);
        CF = _subborrow_u64(CF, r[1], P[1], &r[1]);
        CF = _subborrow_u64(CF, r[2], P[2], &r[2]);
        _subborrow_u64(CF, r[3], P[3], &r[3]);
    }
}
...
{
    mont256_mul(ctx, r, a, (const std::uint64_t*)ctx->R_POW2);
}
                 ...
                 const std::uint64_t addend[4]) noexcept
{
    ...
    if (c || uint256_cmp(sum, P) >= 0)
    {
        c = _subborrow_u64(0, sum[0], P[0], &sum[0]);
        c = _subborrow_u64(c, sum[1], P[1], &sum[1]);
        c = _subborrow_u64(c, sum[2], P[2], &sum[2]);
        c = _subborrow_u64(c, sum[3], P[3], &sum[3]);
    }
}
                 ...
                 const std::uint64_t subtrahend[4]) noexcept
{
    ...
    if (c || uint256_cmp(difference, P) >= 0)
    {
        c = _addcarry_u64(0, difference[0], P[0], &difference[0]);
        c = _addcarry_u64(c, difference[1], P[1], &difference[1]);
        c = _addcarry_u64(c, difference[2], P[2], &difference[2]);
        c = _addcarry_u64(c, difference[3], P[3], &difference[3]);
    }
}
                 ...
                 const std::uint64_t multiplier[4]) noexcept
{
    ...
    if (c || uint256_cmp(product, P) >= 0)
    {
        c = _subborrow_u64(0, product[0], P[0], &product[0]);
        c = _subborrow_u64(c, product[1], P[1], &product[1]);
        c = _subborrow_u64(c, product[2], P[2], &product[2]);
        c = _subborrow_u64(c, product[3], P[3], &product[3]);
    }
}
                 ...
                 const std::uint64_t num[4]) noexcept
{
    ...
    uint256_sub_borrow(ret, (const std::uint64_t*)ctx->P, num);
}
...
void mont256_inv(const Mont256CTX*   ctx,
                 ...
                 const std::uint64_t num[4]) noexcept
{
    mont256_pow(ctx, inverse, num, (const std::uint64_t*)ctx->P_SUB2);
}
...
{
    (void)(ctx); // avoid warning C4100 in MSVC
...
{
    return uint256_equal(a, (const std::uint64_t*)ctx->R);
}
...
{
    uint256_cpy(num, (const std::uint64_t*)ctx->R);
}
...
{
    std::uint64_t t[4];
    uint256_mod(t, bytes, bytes_len, (const std::uint64_t*)ctx->P);
