...
static const std::uint8_t INV_SBOX[256] = {
 ...
 */
static void aes_enc_key_init(std::uint32_t      *round_key,
...
static inline void aes_inv_mixcol(std::uint32_t       out[4],
                                  const std::uint32_t in[4]) noexcept
{
    for (int i = 0; i < 4; i++)
    {
        std::uint32_t si = in[i];
 ...
 * @param bits          AES key bits 128/192/256
 */
static void aes_dec_key_init(std::uint32_t      *round_key,
                             const std::uint8_t *user_key,
                             int                 bits) noexcept
{
    std::uint32_t tmp_round_key[60];
    aes_enc_key_init(tmp_round_key, user_key, bits);
    int nr = 10 + (bits - 128) / 32;
    //------------TransKey-----------
    round_key[0] = tmp_round_key[4 * nr + 0];
    round_key[1] = tmp_round_key[4 * nr + 1];
    round_key[2] = tmp_round_key[4 * nr + 2];
    round_key[3] = tmp_round_key[4 * nr + 3];
    for (int i = 1; i < nr; i++)
    {
        aes_inv_mixcol(round_key + 4 * i, tmp_round_key + 4 * (nr - i));
    }
    round_key[4 * nr + 0] = tmp_round_key[0];
    round_key[4 * nr + 1] = tmp_round_key[1];
    round_key[4 * nr + 2] = tmp_round_key[2];
    round_key[4 * nr + 3] = tmp_round_key[3];
}
 ...
 */
static inline void aes_enc_round(std::uint32_t       s[4],
 ...
 * @param rk    4-dword round key (128-bit)
 */
static inline void aes_enc_last(std::uint32_t       s[4],
                                const std::uint32_t rk[4]) noexcept
{
    std::uint32_t t[4];
    //------------ShiftRow + SubByte-----------
 ...
 * @param rk    4-dword round key (128-bit)
 */
static inline void aes_dec_round(std::uint32_t       s[4],
                                 const std::uint32_t rk[4]) noexcept
{
    std::uint32_t t[4];
 ...
 * @param rk    4-dword round key (128-bit)
 */
static inline void aes_dec_last(std::uint32_t       s[4],
                                const std::uint32_t rk[4]) noexcept
{
    std::uint32_t t[4];
    //------------ShiftRow + SubByte-----------
 ...
 * @param bits          AES key bits 128/192/256
 */
static void aes_enc_block(const std::uint32_t *round_key,
                          ...
                          int                  bits) noexcept
{
    ...
    for (int i = 1; i < nr; i++)
    {
        aes_enc_round(state, round_key + 4 * i);
    }
    aes_enc_last(state, round_key + 4 * nr);
    //-----------Store as BigEndian--------------
 ...
 * @param bits          AES key bits 128/192/256
 */
static void aes_dec_block(const std::uint32_t *round_key,
                          ...
                          int                  bits) noexcept
{
    ...
    for (int i = 1; i < nr; i++)
    {
        aes_dec_round(state, round_key + 4 * i);
    }
    aes_dec_last(state, round_key + 4 * nr);
    //-----------Store as BigEndian--------------
...
static void aes_enc_blocks(const std::uint32_t *round_key,
                           ...
                           int                  bits) noexcept
{
    while (block_num)
    {
        aes_enc_block(round_key, ciphertext, plaintext, bits);
        ciphertext += 16, plaintext += 16, block_num -= 1;
    }
}

static void aes_dec_blocks(const std::uint32_t *round_key,
                           ...
                           int                  bits) noexcept
{
    while (block_num)
    {
        aes_dec_block(round_key, plaintext, ciphertext, bits);
